<template>
    <nav class="flex text-sm text-blue-700 mb-6" aria-label="Breadcrumb">
      <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        <li class="inline-flex items-center">
          <NuxtLink to="/" class="inline-flex items-center text-blue-700 hover:text-blue-800">
        ...
        <li v-for="(crumb, index) in crumbs" :key="index" class="flex items-center">
          <svg class="w-3 h-3 text-blue-400 mx-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
            <path d="M1 9l4-4-4-4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
          <span v-if="crumb.to">
            <NuxtLink :to="crumb.to" class="text-blue-700 hover:text-blue-800">
              {{ crumb.label }}
            </NuxtLink>
          </span>
