<template>
    <div class="mt-10 border-t pt-6">
      <h2 class="text-2xl font-bold mb-6 text-gray-800">Comentaris dels usuaris</h2>
...
>
  ...
  >
    ...
    <div class="flex-1">
      <p class="font-semibold text-gray-800 mb-1 text-sm">{{ comment.user.name }}</p>
      ...
      <div class="mt-8 bg-gray-50 p-6 rounded-xl shadow-inner">
        <div v-if="isLoggedIn">
          ...
          <button
            @click="submitComment"
            class="mt-3 px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md transition"
            :disabled="sending"
          >
        ...
        <div v-else class="text-gray-600">
          <NuxtLink to="/login" class="text-blue-500 underline">Inicia sessió</NuxtLink> per escriure un comentari.
        </div>
      </div>
    </div>
  </template>
  ...
  import { fetchCommentsForTrip, postComment } from '~/services/communicationManager'
  ...
  const submitComment = async () => {
    ...
    try {
      sending.value = true
      await postComment(props.tripId, newComment.value, authStore.token)
      newComment.value = ''
